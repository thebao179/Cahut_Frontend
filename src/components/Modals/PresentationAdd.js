...
function PresentationAdd({preCreate, setPreCreate}) {
    const {
        register,
        handleSubmit,
        formState: {errors}
    } = useForm({
        resolver: yupResolver(PresentationAddSchema)
    });
    ...
    const onSubmit = async (data) => {
        ...
        if (result.status) {
            $('#presentation-add-modal').find('input').val('');
            setPreCreate(preCreate + 1);
        }
    }

    return (
        <div className="modal fade" id="presentation-add-modal" role="dialog"
             aria-labelledby="presentation-add-modal" aria-hidden="true">
                    ...
                    <form onSubmit={handleSubmit(onSubmit)}>
                        <div className="block block-rounded block-transparent mb-0">
                            <div className="block-header block-header-default">
                                <h3 className="block-title">Create new presentation</h3>
