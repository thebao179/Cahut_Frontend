...
CKEDITOR.dialog.add("a11yHelp", function (f) {
  function m(a) {
    for (var b, c, h = [], d = 0; d < g.length; d++)
      (c = g[d]), (b = a / g[d]), b > 1 && b <= 2 && ((a -= c), h.push(e[c]));
    h.push(e[a] || String.fromCharCode(a));
    return h.join("+");
  }
  function t(a, b) {
    const c = f.getCommandKeystroke(b, !0);
    return c.length ? CKEDITOR.tools.array.map(c, m).join(" / ") : a;
  }
  const a = f.lang.a11yhelp;
  const b = f.lang.common.keyboard;
  const p = CKEDITOR.tools.getNextId();
  const q = /\$\{(.*?)\}/g;
  var g = [CKEDITOR.ALT, CKEDITOR.SHIFT, CKEDITOR.CTRL];
  var e = {
    8: b[8],
    9: a.tab,
    13: b[13],
        ...
        elements: [
          {
            ...
            html: `${(function () {
              for (
                var b = `\x3cdiv class\x3d"cke_accessibility_legend" role\x3d"document" aria-labelledby\x3d"${p}_arialbl" tabIndex\x3d"-1"\x3e%1\x3c/div\x3e\x3cspan id\x3d"${p}_arialbl" class\x3d"cke_voice_label"\x3e${a.contents} \x3c/span\x3e`,
                  e = [],
                  c = a.legend,
                  h = c.length,
                  d = 0;
                ...
                for (
                  ...
                  const k = r[n];
                  var l =
                    CKEDITOR.env.edge && k.legendEdge ? k.legendEdge : k.legend;
                  var l = l.replace(q, t);
                  l.match(q) ||
                    g.push(
